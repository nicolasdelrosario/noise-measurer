...
export function MonitorScene({ level, volume, alert, active, status, stateCode, limit, sensitivity, soundLabel, onStart, onStop, onMessage }) {
  ...
  return <article ref={cardRef} className="monitor-card" data-alert={alert} aria-labelledby="monitor-status">
    ...
    <div className="face-field">
      ...
      <div className="fullscreen-status"><strong>{alert ? "Demasiado ruido" : status}</strong><div className="fullscreen-metrics"><span>{active && Number.isFinite(level) ? `${level.toFixed(1)} dB` : "-- dB"}</span><span>Límite {limit} dB</span><span>Sensibilidad {sensitivity}</span><span>{soundLabel}</span></div><button className="fullscreen-exit" type="button" onClick={toggleFullscreen}>Salir</button></div>
    </div>
    <div className="monitor-status-row">
      <div><p className="eyebrow">Estado del aula</p><h2 id="monitor-status">{status}</h2></div>
